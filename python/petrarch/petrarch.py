...
class CheckVerbsError(
    Exception):  # problems were found in a specific pattern in check_verbs [make this local to that function?]
    pass
    ...
    while ke < len(CodedEvents):  # check that all coded events have matches
        ...
        while kv < len(ValidEvents):
            if (len(ValidEvents[kv]) > 3):
                ...
                if (CodedEvents[ke][0] == ValidEvents[kv][0]) and (CodedEvents[ke][1] == ValidEvents[kv][1]) and (
                    CodedEvents[ke][2] == ValidEvents[kv][2]):
...
def get_loccodes(thisloc):
    ...
    def get_ne_text(neloc, isupperseq):
        ...
        if isupperseq:
            ...
            while ka >= 0 and UpperSeq[ka][
                0] != '~':  # we can get an unbalanced sequence when multi-word verbs cut into the noun phrase: see DEMO-30 in unit-tests
                acphr += ' ' + UpperSeq[ka]
                ka -= 1
        else:
...
def get_lower_seq(kword, endtag):
    ...
    while (endtag not in ParseList[kword]):  # limit this to the verb phrase itself
        ...
        if ('(NE' == ParseList[kword]):
            LowerSeq.append(
                ParseList[kword] + '<' + str(kword) + '>' + ParseList[kword + 1])  # <pas 13.07.26> See Note-1
            kword += 1  # skip code
        elif ('NEC' in ParseList[kword]):
                ...
                for words in PETRglobals.VerbDict[patlist[kpatword]]:
                    ...
                    if ' ' in words:  # try to match a phrase
                        ...
                        if isupperseq:
                            ...
                            ka = 0
                            while (ka < len(wordlist)) and ((kseq + ka) < len(aseq)) and (
                                aseq[kseq + ka] == wordlist[ka]):
                                ka += 1
                        if ka == len(wordlist):
...
def check_verbs():
    ...
    def raise_CheckVerbs_error(kloc, call_location_string):
        ...
        warningstr = call_location_string + 'in check_verbs; verb sequence {} skipped: {}'.format(
            ' '.join(ParseList[kloc:kloc + 5]), SentenceID)
        logger = logging.getLogger('petr_log')
        logger.warning(warningstr)
                    ...
                    while (ka < len(patternlist) and not isinstance(patternlist[ka][2], basestring)):
                        ...
                        if make_multi_sequences(patternlist[ka][2], kitem + 2, endtag):
                            ...
                            patternlist = PETRglobals.VerbDict[
                                patternlist[ka][1]]  # redirect to the list for the primary verb
                            break
                        ka += 1
                    else:
                        make_check_sequences(kitem + 2, endtag)
                        if (ka > 2 and ka < len(patternlist) and isinstance(patternlist[ka][2],
                                                                            basestring)):  # use the single word option
    ...
    def make_events(codessrc, codestar, codeevt):
        ...
        for thissrc in codessrc:
            ...
            if srclist[0][0:3] == '---' and len(SentenceLoc) > 0:
                srclist[0] = SentenceLoc + srclist[0][
                                           3:]  # add location if known <14.09.24: this still hasn't been implemented <>
