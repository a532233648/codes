...
def stanford_parse(event_dict):
    ...
    BerkeleyInterface.startup(cp)
    gr = os.environ.get("BERKELEY_PARSER_GRM", GRM_PATH)
    args = {"gr":gr, "tokenize":True}
    opts = BerkeleyInterface.getOpts(BerkeleyInterface.dictToArgs(args))
    parser = BerkeleyInterface.loadGrammar(opts)
    ...
    for i, key in enumerate(event_dict.keys()):
        ...
        for sent in event_dict[key]['sents']:
            ...
            if len(sent_dict['content']) > 512 or len(sent_dict['content']) < 64:
                ...
                try:
                    strIn = StringIO(sent_dict['content'])
                    strOut = StringIO()
                    BerkeleyInterface.parseInput(parser, opts, inputFile=strIn, outputFile=strOut)
                    s_parsetree = strOut.getvalue().replace('(', '(ROOT', 1)
